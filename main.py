...
# Límites semanales definidos por tu regla de negocio
LIMITES_SEMANALES = {
...
# --- MOTOR CON REGLAS DE VARIEDAD ---
def generar_dia_estricto(get_obj, p_obj, c_obj, l_obj, tags_req, historial_semanal, conteo_uso):
    ...
    for _ in range(1000):
        ...
        for j in range(4):
            ...
            # REGLA 1: Filtrar por límite semanal (Soft constraint)
            pool_filtrado = [p for p in pool if conteo_uso.get(p["nombre"], 0) < LIMITES_SEMANALES.get(p["nombre"], 7)]
            
            # REGLA 2: No repetir lo mismo que hace < 48hs (2 días = 8 comidas atrás)
            pool_filtrado = [p for p in pool_filtrado if p["nombre"] not in historial_semanal[-8:]]
            
            # Si los filtros nos dejan vacíos, usamos el pool base para no romper el código
            ...
            historial_temp.append(plato_base["nombre"])
        ...
        if all(error(r, o) <= 0.05 for r, o in zip([tk, tp, tc, tl], [get_obj, p_obj, c_obj, l_obj])):
            # Actualizar conteos globales si el día es válido
            for nombre in historial_temp:
                conteo_uso[nombre] = conteo_uso.get(nombre, 0) + 1
                historial_semanal.append(nombre)
